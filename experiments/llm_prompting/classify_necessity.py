...
def load_system_prompt(prompt_file: Optional[str] = None) -> str:
    ...
    if not prompt_file.exists():
        ...
        return "你是一位隐私政策审计专家，专注于评估必要性违规。"
...
async def classify_from_file(
    ...
    """从文件读取声明并进行必要性分类"""
    logger.info(f"[必要性分类器] 读取输入文件: {input_file}")
    ...
    logger.info(f"[必要性分类器] 共读取 {len(statements)} 条声明")
    ...
    logger.info(f"[必要性分类器] 完成! 总数: {stats['total']}, 成功: {stats['successful']}, 失败: {stats['failed']}")
    logger.info(f"[必要性分类器] 结果已保存到: {output_file}")
    return stats
...
async def run_example():
    ...
    output_file = output_dir / "llm_necessity_results.jsonl"
